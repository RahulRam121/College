...
def Astar(init_state):
    ...
    while(len(open_list) != 0):
        s, open_list = deleteMin(open_list, g, f)
        f = findOutofOrder(s) + g
...
def main():
    ...
    path = Astar(init_state)
    index = 0
    print(path[0], end = '->')
    for i in range(len(path)-1):
        if path[i] in getSuccessors(path[index]):
            index = i
            print(path[i], end = '->')
...
"""
[5, 4, 3, 2, 1]->[3, 2, 5, 4, 1] ->[3, 2, 1, 4, 5] ->[1, 2, 3, 4, 5]
"""
